<div>
    <flux:modal name="crear-servicio" class="md:w-1/2">
        <form wire:submit.prevent="guardar" class="space-y-6">
            ...
            @error('router') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror

            <!-- Litebean -->
            <flux:input label="Litebean" wire:model="litebean" required />
            @error('litebean') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
            ...
            @error('falla_reportada') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
            ...
            <div>
                ...
                @error('tecnico_id') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
            </div>

            <!-- Botones -->
            <div class="flex gap-4 pt-4">
                <flux:button type="button" variant="outline" @click="$dispatch('close-modal', 'crear-servicio')" class="flex-1">
                    Cancelar
                </flux:button>
